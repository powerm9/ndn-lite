...
static int ndn_udp_face_up(struct ndn_face_intf* self);

static int ndn_udp_face_down(struct ndn_face_intf* self);

static void ndn_udp_face_destroy(struct ndn_face_intf* self);

static int ndn_udp_face_send(ndn_face_intf_t* self, const uint8_t* packet, uint32_t size);

static ndn_udp_face_t* ndn_udp_face_construct(
...
static void ndn_udp_face_recv(void *self, size_t param_len, void *param);
...
static int ndn_udp_face_send(ndn_face_intf_t* self, const uint8_t* packet, uint32_t size) {
    ...
    if(ret != size){
        ...
        #ifdef DEBUG_UDP_FACE
          printf("UDP Face [%u] sent packet: ", self->face_id);
          for (uint32_t i = 0; i < size; i++) {
              printf("%02X ", packet[i]); // prints each byte as two-digit hex
          }
          printf("[%.*s]\n", (int)size, packet);
...
ndn_udp_face_t* ndn_udp_unicast_face_construct(
  in_addr_t local_addr,
  in_port_t local_port,
  in_addr_t remote_addr,
  in_port_t remote_port)
